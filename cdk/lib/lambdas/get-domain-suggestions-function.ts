...
> = async payload => {
  ...
  const suggestionCount = payload.queryStringParameters?.suggestionCount
    ? parseInt(payload.queryStringParameters.suggestionCount, 10)
    : undefined;

  if (!domainName) {
    throw new HttpError(
      "domainName is required and must be a non-empty string query parameter.",
      400,
    );
  }
  ...
  try {
    ...
    console.error(
      `Error getting domain suggestions for "${domainName}":`,
      error,
    );
    ...
    throw new HttpError(
      `Failed to get domain suggestions: ${message}`,
      500,
    );
  }
};
