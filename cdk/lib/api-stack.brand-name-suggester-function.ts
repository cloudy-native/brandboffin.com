...
let anthropic: Anthropic | undefined; // Renamed from anthropicClient for clarity with SDK
...
async function getApiKey(secretName: string): Promise<string> {
  ...
  try {
    ...
    if (data.SecretString) {
      ...
      throw new Error("'apiKey' field not found or not a string in secret JSON.");
    }
    throw new Error("API key not found in secret string (SecretString is empty).");
  } catch (error: any) {
    console.error(`Error retrieving API key '${secretName}':`, error.message, error);
    throw new HttpError(
...
async function initializeAnthropicClient(): Promise<Anthropic> {
  ...
  if (!claudeSecretName) {
    ...
    throw new HttpError("Server configuration error: CLAUDE_SECRET_NAME not set.", 500);
  }
...
function constructClaudePrompt(
  ...
  const { prompt, industry, style, keywords, length, count = defaultCount } = request;
  ...
  if (keywords && keywords.length > 0) fullPrompt += `- Keywords: ${keywords.join(", ")}\n`;
...
const brandNameSuggesterLogic: CoreLambdaLogic<RequestBody, ResponseBody> = async (payload) => {
  if (!payload.body || !payload.body.prompt || typeof payload.body.prompt !== 'string' || !payload.body.prompt.trim()) {
    throw new HttpError("Prompt is required and must be a non-empty string", 400);
  }
  ...
  const systemPromptContent = "You are an expert brand name generator. Your task is to provide creative and relevant brand name suggestions based on user input. Output only the names, each on a new line.";
  ...
  try {
    ...
    const messages: MessageParam[] = [{ role: "user", content: userPromptContent }];
    ...
    if (response.content && response.content.length > 0 && response.content[0].type === 'text') {
        suggestions = response.content[0].text
            .split('\n')
            .map(s => s.trim())
            .filter(s => s.length > 0 && !s.startsWith("Here are") && !s.startsWith("Okay, here") && !s.match(/^\d+\.\s/)); // Filter out common conversational fluff and numbered lists
    }
    ...
    throw new HttpError(error.message || 'Failed to generate brand name suggestions', error.status || 500);
  }
};
